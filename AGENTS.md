...
| `needs_review` | A value read back changed, a stranger's contact details are on it, the design has no field Gable recognises, or Slides would not return the text — the four stops that survive the 2026-09-01 policy | Carmen or Chase resolves the named problem and requests a recheck or retry. A flyer that DOES exist is otherwise always sent: an unfilled slot, an unplaced image, a moved rectangle and every vision finding are notes under the link — see below |
