...
def main() -> int:
    ...
    parser.add_argument("--db", default="", help="database path; defaults to GABLE_DB_PATH")
    args = parser.parse_args()

    load_dotenv()
    db_path = args.db or os.environ.get("GABLE_DB_PATH", "gable.db")
    connection = connect(Path(db_path))
    apply_migrations(connection)

    try:
        client = SheetClient.build(
            os.environ["GOOGLE_SERVICE_ACCOUNT_FILE"], os.environ["GABLE_SHEET_ID"]
        )
        submissions = repo.read_submissions(client, os.environ["GABLE_TAB_RESPONSES"])
